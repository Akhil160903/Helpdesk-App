...
import DropdownComponent from './dropdown_sp';
import moment from 'moment';

const TicketPage = ({ route }) => {
  const { id, desc, summary, createdOn, user, status } = route.params;
...
const TicketCard = ({ ticket, onSave }) => {
  ...
  const { assignTicket, openTicket,editTicketStatusSuccess } = useContext(TicketContext);
  const { closeTicket, closeTicketSuccess } = useContext(TicketContext);
  const navigation = useNavigation();


  const assignTicketSp = (sp_email) => {
    setStatusFlag(true);
    setEmail(sp_email);
  }
  ...
  return (
    <ScrollView>
      <View style={styles.container}>
        ...
        <CustomButton onPress={handleEdit} text="Assign Ticket" disabled={
          ticket.status === 'Closed' || ticket.status === 'Assigned' || ticket.status === 'WIP' || ticket.status === 'Resolved'
        }>
        </CustomButton>
        <CustomButton onPress={handleOpenTicket} bgColor={'#f00fe2'} text={"Unassign Ticket"} disabled={
          ticket.status !== 'Assigned' && ticket.status !== 'WIP'
        }>
        </CustomButton>
      ...
      <View style={styles.details}>
        ...
        <Text style={styles.assignedTo}>Created by: {ticket.user}</Text>
