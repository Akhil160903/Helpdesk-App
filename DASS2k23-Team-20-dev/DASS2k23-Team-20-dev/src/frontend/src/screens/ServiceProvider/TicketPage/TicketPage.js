...
const TicketCard = ({ ticket, onSave }) => {
  ...
  const { setWIPforTicket, resolveTicket, editTicketStatusSuccess } = useContext(TicketContext);
  const navigation = useNavigation();


  const changeStatus = (status) => {
    setEditedTicket({ ...ticket, status: status });
    setStatusFlag(true);
    setStatus(status);
  }
  ...
  return (
    <ScrollView>
      <View style={styles.container}>
        ...
        <CustomButton onPress={handleEdit} text="Edit Status" disabled={
